...
import { TextField, FormControl, Autocomplete, Stack, Box } from "@mui/material";
import { ProfileDataContext } from '../../Context/CompleteProfileContext';
import useAxios from "../utilities/useAxios";
import { baseURL } from "../utilities/constants";
import { AuthContext } from "../../Context/AuthProvider";

export default function PersonalDetails({ resetProfileForm }){
    const { profileData } = React.useContext(ProfileDataContext);
    let { setProfileCompleted } = React.useContext(AuthContext);

    const [ firstName , setFirstName ] = React.useState(profileData?.first_name);
    const [ middleName , setMiddleName ] = React.useState(profileData?.middle_name);
    const [ lastName , setLastName ] = React.useState(profileData?.last_name);
    const [ mobile , setMobile ] = React.useState(profileData?.mobile);
    const [ mobileCountryCode , setCountryCode ] = React.useState(profileData?.countryCode ? profileData?.countryCode : countryCode[0].code);
    const [ dob , setDob ] = React.useState(profileData?.date_of_birth);
    const [ email , setEmail ] = React.useState(profileData?.email);

    const API = useAxios();

    const handleFirstNameChange = (event) =>{
        profileData.first_name = event.target.value;
        setFirstName(event.target.value)
    }
    const handleMiddleNameChange = (event) =>{
        profileData.middle_name = event.target.value;
        setMiddleName(event.target.value)
    }
    const handleLastNameChange = (event) =>{
        profileData.last_name = event.target.value;
        setLastName(event.target.value)
    }
    const handleMobileChange = (event) =>{
        profileData.mobile = event.target.value;
        setMobile(event.target.value)
    }
    const handleMobileCountryCodeChange = (event) => {
        profileData.countryCode = event.target.value;
        setCountryCode(event.target.value)
    }
    const handleDobChange = (event) =>{
        profileData.date_of_birth = event.target.value;
        ...
        API.get(`${baseURL}/v2/getUserDetails/`)
            .then(resp => {
                if (resp.data["task_completed"]) {
                    profileData.first_name = resp.data["first_name"];
                    profileData.last_name = resp.data["last_name"];
                    profileData.email = resp.data["email"];
                ...
                <FormControl sx={{ minWidth: 300, display: "flex", flexDirection:"row", gap: 1 }}>
                    <Stack sx={{ maxWidth: 100 }}>
                        <Autocomplete 
                            ...
                            noOptionsText={"No available country code"}
                            onChange={handleMobileCountryCodeChange}
                            renderOption={(props, countryCode) => (
                                <Box component="li" {...props} value={countryCode.code} key={`${countryCode.country}-${countryCode.code}`}>
